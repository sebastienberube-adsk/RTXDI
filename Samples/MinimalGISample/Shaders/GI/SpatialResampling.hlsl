...
void main(uint2 GlobalIndex : SV_DispatchThreadID)
{
    ...
    RTXDI_GIReservoir reservoir = RTXDI_LoadGIReservoir(g_Const.restirGI.reservoirBufferParams,
        GlobalIndex, g_Const.restirGI.bufferIndices.spatialResamplingInputBufferIndex);
    ...
    if (RAB_IsSurfaceValid(primarySurface))
    {
        ...
        reservoir = RTXDI_GISpatialResampling(pixelPosition, primarySurface, reservoir, rng,
            g_Const.runtimeParams, g_Const.restirGI.reservoirBufferParams, sparams);
    }

    RTXDI_StoreGIReservoir(reservoir, g_Const.restirGI.reservoirBufferParams,
        GlobalIndex, g_Const.restirGI.bufferIndices.spatialResamplingOutputBufferIndex);
}
